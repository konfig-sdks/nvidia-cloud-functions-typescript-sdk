...
import { readableStreamToString, NvidiaCloudFunctionsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NvidiaCloudFunctionsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NvidiaCloudFunctionsError(e, e.response?.data)
                }
                if (innerError instanceof NvidiaCloudFunctionsError) {
                    // Got "NvidiaCloudFunctionsError" from the above try block
                    throw innerError;
                }
